 ...
 *
 * <p>Paths are matched from the root down. If a user-specified path ends at a non-leaf node, the
 * rest of the branch from that node to the leaf is not retained.
 *
...
public class RetainPathsJsonAction implements JSONTraverseAction {
  ...
  /**
   * if the full path to the entry is not contained in any of the paths to retain - remove it from
   * the object this step does not remove entries whose full path is contained in a path to retain
   * but are not equal to an entry to retain
   *
