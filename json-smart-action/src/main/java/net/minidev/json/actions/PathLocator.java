 ...
 *
 * <p>Traverses the specified {@link JSONObject} searching for nodes whose paths (from the root
 * down) match any of the user-specified paths. The paths that match are returned.
 *
 * <p>A path to locate must be specified in the n-gram format - a list of keys from the root down
...
public class PathLocator {
  ...
  public PathLocator(List<String> pathsToFind) {
    this.pathsToFind =
        pathsToFind == null || pathsToFind.size() == 0
  ...
  public PathLocator(String... pathsToFind) {
    this.pathsToFind =
        pathsToFind == null || pathsToFind.length == 0
            ? Collections.<String>emptyList()
            : Arrays.asList(pathsToFind);
  }
