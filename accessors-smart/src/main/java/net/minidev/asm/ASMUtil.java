...
public class ASMUtil {
  ...
  public static Accessor[] getAccessors(Class<?> type, FieldFilter filter) {
    ...
    if (filter == null) filter = BasicFiledFilter.SINGLETON;
  ...
  /**
   * Append the call of proper extract primitive type of an boxed object. this method use Number
   * interface to unbox object
   *
   * @param mv MethodVisitor
   * @param fieldType expected class
   */
  ...
  public static Label[] newLabels(int cnt) {
    ...
    for (int i = 0; i < cnt; i++) r[i] = new Label();
    return r;
  }
