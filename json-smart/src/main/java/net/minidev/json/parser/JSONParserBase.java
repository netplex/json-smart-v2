...
abstract class JSONParserBase {
  ...
  public JSONParserBase(int permissiveMode) {
    ...
    this.checkTaillingData =
        (permissiveMode & (JSONParser.ACCEPT_TAILLING_DATA | JSONParser.ACCEPT_TAILLING_SPACE))
            != (JSONParser.ACCEPT_TAILLING_DATA | JSONParser.ACCEPT_TAILLING_SPACE);
  ...
  public void checkControleChar() throws ParseException {
    if (ignoreControlChar) return;
    int l = xs.length();
    for (int i = 0; i < l; i++) {
      char c = xs.charAt(i);
      if (c < 0) continue;
      if (c <= 31) throw new ParseException(pos + i, ParseException.ERROR_UNEXPECTED_CHAR, c);
      if (c == 127) {
        if (reject127) throw new ParseException(pos + i, ParseException.ERROR_UNEXPECTED_CHAR, c);
      }
    }
  }
  ...
  public void checkLeadinZero() throws ParseException {
    ...
    if (len == 1) return;
    if (len == 2) {
      if (xs.equals("00")) throw new ParseException(pos, ERROR_UNEXPECTED_LEADING_0, xs);
      return;
    }
    char c1 = xs.charAt(0);
    char c2 = xs.charAt(1);
  ...
  protected Number extractFloat() throws ParseException {
    ...
    try {
      if (!useHiPrecisionFloat) return Float.parseFloat(xs);
  ...
  /**
   * parse from the current position <br>
   * use to return Primitive Type, or String, Or JsonObject or JsonArray generated by a
   * ContainerFactory
   */
  ...
  private <T> T parseInner(JsonReaderI<T> mapper) throws ParseException {
    ...
    try {
      ...
      if (checkTaillingData) {
        if (!checkTaillingSpace) skipSpace();
        if (c != EOI) throw new ParseException(pos - 1, ERROR_UNEXPECTED_TOKEN, c);
      }
    } catch (IOException e) {
  ...
  protected Number parseNumber(String s) throws ParseException {
    ...
    if (mustCheck) {
      ...
      if (r > -922337203685477580L) {
        ...
        if (neg) isBig = (s.charAt(p) > '8');
        else isBig = (s.charAt(p) > '7');
      }
      if (isBig) return new BigInteger(s, 10);
      r = r * 10L + ('0' - s.charAt(p));
    }
    if (neg) {
      if (this.useIntegerStorage && r >= Integer.MIN_VALUE) return (int) r;
      return r;
    }
    r = -r;
    if (this.useIntegerStorage && r <= Integer.MAX_VALUE) return (int) r;
    return r;
  }
  ...
  protected <T> T readArray(JsonReaderI<T> mapper) throws ParseException, IOException {
    if (c != '[') throw new RuntimeException("Internal Error");
  ...
  protected <T> T readFirst(JsonReaderI<T> mapper) throws ParseException, IOException {
    for (; ; ) {
      switch (c) {
        ...
        case 'N':
          readNQString(stopX);
          if (!acceptNaN) throw new ParseException(pos, ERROR_UNEXPECTED_TOKEN, xs);
          if ("NaN".equals(xs)) {
            //
   ...
   * ContainerFactory
   */
  protected Object readMain(JsonReaderI<?> mapper, boolean stop[])
    ...
    for (; ; ) {
      ...
      switch (c) {
        ...
        case (char) 31: // Unit Separator
          if (ignoreControlChar) continue;
          throw new ParseException(pos, ERROR_UNEXPECTED_CHAR, c);
        case (char) 127: // del
          if (ignoreControlChar) continue;
          if (reject127) throw new ParseException(pos, ERROR_UNEXPECTED_CHAR, c);
        default:
          sb.append(c);
      }
    }
  ...
  protected char readUnicode(int totalChars) throws ParseException, IOException {
    ...
    for (int i = 0; i < totalChars; i++) {
      ...
      else if (c == EOI) throw new ParseException(pos, ERROR_UNEXPECTED_EOF, "EOF");
      else throw new ParseException(pos, ERROR_UNEXPECTED_UNICODE, c);
    }
    return (char) value;
  }
  ...
  protected void skipNQString(boolean[] stop) throws IOException {
    for (; ; ) {
      if ((c == EOI) || (c >= 0 && c < MAX_STOP && stop[c])) return;
      readS();
    }
  }
