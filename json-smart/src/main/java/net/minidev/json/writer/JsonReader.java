...
public class JsonReader {
  ...
  public <T> JsonReaderI<T> getMapper(Type type) {
    if (type instanceof ParameterizedType) return getMapper((ParameterizedType) type);
  ...
  public <T> JsonReaderI<T> getMapper(Class<T> type) {
    ...
    if (type instanceof Class) {
      if (Map.class.isAssignableFrom(type)) map = new DefaultMapperCollection<T>(this, type);
      else if (List.class.isAssignableFrom(type)) map = new DefaultMapperCollection<T>(this, type);
      if (map != null) {
        cache.put(type, map);
        return map;
      }
    ...
    if (type.isArray()) map = new ArraysMapper.GenericMapper<T>(this, type);
    else if (List.class.isAssignableFrom(type)) map = new CollectionMapper.ListClass<T>(this, type);
    else if (Map.class.isAssignableFrom(type)) map = new CollectionMapper.MapClass<T>(this, type);
    else
      // use bean class
  ...
  public <T> JsonReaderI<T> getMapper(ParameterizedType type) {
    ...
    if (List.class.isAssignableFrom(clz)) map = new CollectionMapper.ListType<T>(this, type);
    else if (Map.class.isAssignableFrom(clz)) map = new CollectionMapper.MapType<T>(this, type);
    cache.putIfAbsent(type, map);
    return map;
  }
}
