...
public class CollectionMapper {
  ...
  public static class MapType<T> extends JsonReaderI<T> {
    ...
    public MapType(JsonReader base, ParameterizedType type) {
      ...
      if (rawClass.isInterface()) instance = JSONObject.class;
      else instance = rawClass;
      ...
      else keyClass = (Class<?>) ((ParameterizedType) keyType).getRawType();
      if (valueType instanceof Class) valueClass = (Class<?>) valueType;
      else valueClass = (Class<?>) ((ParameterizedType) valueType).getRawType();
    }
    ...
    public void setValue(Object current, String key, Object value) {
      ((Map<Object, Object>) current)
          .put(JSONUtil.convertToX(key, keyClass), JSONUtil.convertToX(value, valueClass));
    }
  ...
  public static class MapClass<T> extends JsonReaderI<T> {
    ...
    public MapClass(JsonReader base, Class<?> type) {
      super(base);
      this.type = type;
      if (type.isInterface()) this.instance = JSONObject.class;
      else this.instance = type;
  ...
  public static class ListType<T> extends JsonReaderI<T> {
    ...
    public ListType(JsonReader base, ParameterizedType type) {
      ...
      this.type = type;
      this.rawClass = (Class<?>) type.getRawType();
      if (rawClass.isInterface()) instance = JSONArray.class;
      else instance = rawClass;
  ...
  public static class ListClass<T> extends JsonReaderI<T> {
    ...
    public ListClass(JsonReader base, Class<?> clazz) {
      super(base);
      this.type = clazz;
      if (clazz.isInterface()) instance = JSONArray.class;
      else instance = clazz;
      ba = BeansAccess.get(instance, JSONUtil.JSON_SMART_FIELD_FILTER);
    }
